    ...
    <main class="p-6">
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100 mb-4">
            <div class="flex flex-col lg:flex-row md:items-left md:justify-between gap-4">
                <div>
                    <h2 class="text-2xl font-bold text-gray-800">Approval Flow</h2>
                    <p class="text-sm text-gray-500 mt-1">Manage and view all approval flow in the system</p>
...
@push('custom-scripts')
    ...
    <script>
        ...
        const routes = {
            add     : "{{ route('admin.setting.approval.flow.store') }}",
            edit    : "{{ route('admin.setting.approval.flow.edit', ['id' => ':id']) }}",
            show    : "{{ route('admin.setting.approval.flow.edit', ['id' => ':id']) }}",
            delete  : "{{ route('admin.setting.approval.flow.delete', ['id' => ':id']) }}",
            assign  : "{{ route('admin.setting.approval.flow.steps.index', ['id' => ':id']) }}"
        };
        ...
        // $(function() {
            ...
            function loadTable(page = 1, start= '' , end = '') {
                $.ajax({
                    url: "{{ route('admin.setting.approval.flow.data') }}",
                    type: "GET",
                    data: {
                ...
                if (data && data.length === 0) {
                    ...
                    data.forEach((item, i) => {
                        ...
                        <tr class="border-b border-gray-200 hover:bg-blue-50 transition-colors">
                            ...
                            <td class="px-6 py-4 border-r border-gray-200">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${item.is_active == 1 ? 'text-green-700 bg-green-100' : 'text-red-700 bg-red-100'} ">
                ...
                function generateDropdown(item) {
                    ...
                    if(userPerms.includes(`edit-${menu}`)){
                        ...
                        <a href='#' data-id="${item.id}" data-type="assign" class="btn-action flex items-center gap-3 px-4 py-3 text-sm text-gray-700 hover:bg-yellow-50 transition-colors">
                            ...
                            <span class="font-medium">Assign Steps</span>
                        </a>
                        `;
                    }
                    ...
                    $.ajax({
                        ...
                        success: function(res) {
                            ...
                            if(type == 'edit'){
                                ...
                                const isActiveValue = res.is_active == 1 ? 'true' : 'false';
                                $('#isActive').val(isActiveValue).change();
                                $('#description').val(res.description);

                                // const approvalBy = res.approval_by;
                                // $('#approval_by').val(approvalBy).change();
                                // $('#description').val(JSON.stringify(res.description, null, 2));
                                ...
                                // form Modal
                                let urlEdit = '{{ route('admin.setting.approval.flow.update', ['id' => ':idx']) }}'.replace(':idx', idx);
                                ...
                                $('#name').val(res.name);
                                $('#module').val(res.module);
                                const isActiveValue = res.isActiveValue == 1 ? 'true' : 'false';
                                $('#isActive').val(isActiveValue).change();
                                $('#description').val(res.description);
