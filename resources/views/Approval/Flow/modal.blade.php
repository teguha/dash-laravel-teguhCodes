    ...
    <div id="modalContainer" class="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 transform transition-all duration-300 scale-95 opacity-0">
        ...
        <form id="formModal" data-type="add" data-url="#">
            ...
            <div class="p-6 space-y-5" id="form-body">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2 text-[14px]">
                        <i class="fas fa-user text-gray-400 mr-2"></i>Module
                    </label>
                    <input type="text" placeholder="Module" id="module" name="module" class="w-full px-4 py-3 border border-gray-300 text-[12px] rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 outline-none" required>
                </div>

                {{-- @php 
                    $parents =  App\Models\Master\Structure::where('level', 'direksi')->get();
                @endphp --}}

                {{-- description --}}
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2 text-[14px]">
                        <i class="fas fa-envelope text-gray-400 mr-2"></i>Description
                    </label>
                    <textarea  id="description" name="description" class="w-full px-4 py-3 border border-gray-300 text-[12px] rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 outline-none h-40"></textarea>
                </div>
                ...
                <div class="max-w-md">
                    ...
                    <select id="isActive" name="is_active" class="w-full" required>
