...
@section('content')
    <main class="p-6">
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100 mb-4">
            <div class="flex flex-col lg:flex-row md:items-left md:justify-between gap-4">
                <div>
                    <h2 class="text-2xl font-bold text-gray-800">Approval Flow Steps</h2>
                    <p class="text-sm text-gray-500 mt-1">Manage and view all approval flow steps in the system</p>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            @php
                ...
                $columns = [
                    ...
                    ['field' => 'approval_type', 'label' => 'Approval Type', 'sortable' => true],
                    ['field' => 'min_approve',  'label' => 'Min Approve', 'sortable' => false],
    ...
    {{-- modal --}}
    @include('Approval.FlowSteps.modal')
...
@push('custom-scripts')
    ...
    <script>
        ...
        const routes = {
            add     : "{{ route('admin.setting.approval.flow.steps.store') }}",
            edit    : "{{ route('admin.setting.approval.flow.steps.edit', ['id' => ':id']) }}",
            show    : "{{ route('admin.setting.approval.flow.steps.edit', ['id' => ':id']) }}",
            delete  : "{{ route('admin.setting.approval.flow.steps.delete', ['id' => ':id']) }}",
            assign  : "{{ route('admin.setting.approval.flow.steps.approver.index', ['id' => ':id']) }}",
        };
        ...
        // $(function() {
            ...
            function loadTable(page = 1, start= '' , end = '') {
                let url_get = '{{ route('admin.setting.approval.flow.steps.data', ['id' => ':idData']) }}'.replace(':idData', idData);
                $.ajax({
                    url: url_get,
                    type: "GET",
                    data: {
            ...
            function renderTable(data, pagination) {
                ...
                if (data && data.length === 0) {
                    ...
                    data.forEach((item, i) => {
                        ...
                        <tr class="border-b border-gray-200 hover:bg-blue-50 transition-colors">
                            ...
                            <td class="px-6 py-4 border-r border-gray-200">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${item.approval_type == 'Sequence' ? 'text-green-700 bg-green-100' : 'text-red-700 bg-red-100'} ">
                                    ${item.approval_type}
                                </span>
                            </td>

                            <td class="px-6 py-4 border-r border-gray-200">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${item.min_approve > 0 ? 'text-green-700 bg-green-100' : 'text-red-700 bg-red-100'} ">
                                    ${item.min_approve}
                                </span>
                            </td>
                ...
                function generateDropdown(item) {
                    ...
                    if(userPerms.includes(`edit-${menu}`)){
                        ...
                        <a href='#' data-id="${item.id}" data-type="assign" class="btn-action flex items-center gap-3 px-4 py-3 text-sm text-gray-700 hover:bg-yellow-50 transition-colors">
                            <div class="w-8 h-8 bg-purple-100 rounded-lg flex items-center justify-center">
                                <i class="fas fa-edit text-purple-600 text-xs"></i>
                            </div>
                            <span class="font-medium">Assign Approver</span>
                        </a>
                    ...
                    $.ajax({
                        ...
                        success: function(res) {
                            ...
                            if(type == 'edit'){
                                ...
                                // form modal
                                $('#step_order').val(res.step_order);
                                $('#min_approve').val(res.min_approve);
                                const approvalType = res.approval_type;
                                $('#approval_type').val(approvalType).change();
                                // const approvalBy = res.approval_by;
                                // $('#approval_by').val(approvalBy).change();
                                ...
                                // form Modal
                                let urlEdit = '{{ route('admin.setting.approval.flow.steps.update', ['id' => ':idx']) }}'.replace(':idx', idx);
    ...
    <script>
        $(document).ready(function() {
            ...
            // $('#approval_by').on('change', function(){
