        ...
        <form id="formModal" data-type="add" data-url="#">
            ...
            <div class="p-6 space-y-5" id="form-body">
                ...
                @php
                    ...
                    $current_step = ApprovalFlowSteps::where('approval_flow_id', $id)->count();
                    $next_steps = $current_step > 0 ? $current_step + 1 : 1;
                @endphp
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2 text-[14px]">
                        <i class="fas fa-user text-gray-400 mr-2"></i>Step Order
                    </label>
                    <input type="number" placeholder="Step Order" value="{{ $next_steps }}" id="step_order" name="step_order" min="1" class="w-full px-4 py-3 border border-gray-300 text-[12px] rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 outline-none" required>
                </div>

                <!-- Module  -->
                <input type="hidden" name="approval_flow_id" value="{{ $id }}">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2 text-[14px]">
                        <i class="fas fa-user text-gray-400 mr-2"></i>Minimum Approve
                    </label>
                    <input type="number" placeholder="Minimum Approve" id="min_approve" name="min_approve" min="1" class="w-full px-4 py-3 border border-gray-300 text-[12px] rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 outline-none" required>
                    <p class="text-xs text-gray-500">Ignore if sequence approval type </p>
                </div>
