<div id="modalOverlay" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50 backdrop-blur-sm transition-opacity duration-300">
    ...
    <div id="modalContainer" class="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 transform transition-all duration-300 scale-95 opacity-0">
        ...
        <!-- Modal Body -->
        <form id="formModal" class="h-[30vh] overflow-y-auto" data-type="add" data-url="#">
            @csrf
            ...
            <div class="p-6 space-y-5" id="form-body">
                ...
                <!-- Structure  -->
                <div id="structure_field" class="hidden">
                    <label class="block text-sm font-medium text-gray-700 mb-2 text-[14px]">
                        <i class="fas fa-user text-gray-400 mr-2"></i>Structure
                    </label>
                    <input type="text" placeholder="Structure" id="structure" name="structure" class="w-full px-4 py-3 border border-gray-300 text-[12px] rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 outline-none" >
                </div>

                <input type="hidden" name="position" id="position" value="{{Auth::user()->struct_id}}">

                <!-- Date  -->
                <div id="date_field" class="hidden">
                    <label class="block text-sm font-medium text-gray-700 mb-2 text-[14px]">
                        <i class="fas fa-user text-gray-400 mr-2"></i>Date
                    </label>
                    <input type="text" placeholder="Date" id="date" name="date" class="w-full px-4 py-3 border border-gray-300 text-[12px] rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 outline-none">
                </div>

                <!-- No  -->
                <div id="no_field" class="hidden">
                    <label class="block text-sm font-medium text-gray-700 mb-2 text-[14px]">
                        <i class="fas fa-user text-gray-400 mr-2"></i>No
                    </label>
                    <input type="text" placeholder="Nomor Surat" id="no" name="no" class="w-full px-4 py-3 border border-gray-300 text-[12px] rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 outline-none" >
                </div>

                @php
                    use App\Models\ApprovalFlow;
                    $module = ApprovalFlow::where('module', 'like', '%perencanaan%')->get();
                @endphp
                ...
                <div class="max-w-md">
                    <label class="custom-label">
                        ...
                        Module Approval Name
                    </label>
                    <select id="module" name="module" class="w-full" required>
