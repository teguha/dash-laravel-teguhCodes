...
class DireksiController extends BaseController
{
    ...
    {
        return view('Struct.Direksi.index'); // blade utama
    }
    ...
    public function getData(Request $request)
    {
        ...
        if($sortBy == 'parent'){
            $data = $query->where('level', 'main_corp')->orderBy('name', $sortDir)->paginate($perPage, ['*'], 'page', $page);
        }else{
            // Jalankan query dengan orderBy, baru paginate
            $data = $query->where('level', 'direksi')->orderBy($sortBy, $sortDir)->paginate($perPage, ['*'], 'page', $page);
        }
